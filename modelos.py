            ...
            for k in Jobs:
                if j<k:
                    for u in Jobs:
                        if u<j<k:
                            # para j precedendo k
                            #modelo.add_constraint(s[i,u,j,k] <=  (1-z[i,u,j])+(1-z[i,j,k]))
                            #modelo.add_constraint(s[i,u,j,k] <=  z[i,u,k]+(1-z[i,j,k]))
                            modelo.add_constraint(s[i,u,j,k] >=  (1-z[i,u,j]) + z[i,u,k] - 1 - (1-z[i,j,k]))
                            # para k precendo j
                            #modelo.add_constraint(s[i,u,j,k] <=  (1-z[i,u,k])+z[i,j,k])
                            #modelo.add_constraint(s[i,u,j,k] <=  z[i,u,j]+z[i,j,k])
                            modelo.add_constraint(s[i,u,j,k] >=  (1-z[i,u,k]) + z[i,u,j] - 1 -z[i,j,k])
                        elif j<u<k:
                            # para j precedendo k
                            #modelo.add_constraint(s[i,u,j,k] <=  z[i,j,u]+(1-z[i,j,k]))
                            #modelo.add_constraint(s[i,u,j,k] <=  z[i,u,k]+(1-z[i,j,k]))
                            modelo.add_constraint(s[i,u,j,k] >=  z[i,j,u] + z[i,u,k] - 1 - (1-z[i,j,k]))    
                            # para k precendo j
                            #modelo.add_constraint(s[i,u,j,k] <=  (1-z[i,u,k])+z[i,j,k])
                            #modelo.add_constraint(s[i,u,j,k] <=  (1-z[i,j,u])+z[i,j,k])
                            modelo.add_constraint(s[i,u,j,k] >=  (1-z[i,u,k])+(1-z[i,j,u]) - 1 - z[i,j,k])
                        elif j<k<u:
                            # para j precedendo k
                            #modelo.add_constraint(s[i,u,j,k] <=  z[i,j,u]+(1-z[i,j,k]))
                            #modelo.add_constraint(s[i,u,j,k] <=  (1-z[i,k,u])+(1-z[i,j,k]))
                            modelo.add_constraint(s[i,u,j,k] >=  z[i,j,u] + (1-z[i,k,u]) - 1 - (1-z[i,j,k]))    
                            # para k precendo j
                            #modelo.add_constraint(s[i,u,j,k] <=  z[i,k,u]+z[i,j,k])
                            #modelo.add_constraint(s[i,u,j,k] <=  (1-z[i,j,u])+z[i,j,k])
                            modelo.add_constraint(s[i,u,j,k] >=  z[i,k,u]+(1-z[i,j,u]) - 1-  z[i,j,k])
...
def jsp_disjuntivo_minla_favorito(tempo, ordem, tempo_max=3600, fl_inteiro=True):
    ...
    cmax = jsp_manne_var_cmax(modelo, Problema) # makespan
    y = None
    s    = jsp_minla_var_s(modelo, Problema)
    ###########################################################################
